    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const userRes = await fetch(
          `http://localhost:5000/user/full-info/email/${userEmail}`,
          {
            headers: {
        ...
        const finesRes = await fetch(
          `http://localhost:5000/user/fine/email/${userEmail}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        const historyRes = await fetch(
          `http://localhost:5000/user/borrowing-history/email/${userEmail}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <div className="dashboard-cards-container">
          ...
          <div className="dashboard-card">
            ...
            <p>
              ...
              {userInfo?.Subscription_Type === 1 ? "VIP" : "Member"}
            </p>
            <p>
              <strong>Expiry Date:</strong> {formatDate(userInfo?.Expiry_Date)}
            </p>
          </div>
          ...
          <div
            ...
            onClick={goToFinesDetails}
            style={{ cursor: "pointer" }}
          >
            <h2>💰 Fines Summary</h2>
            <p>
              <strong>Total Due:</strong> ₹{fines}
            </p>
            <p>
          ...
          >
            ...
            {borrowHistory.length > 2 && (
              <p>
                <small>
                  +{borrowHistory.length - 2} more... (Click to view all)
                </small>
              </p>
            )}
            {borrowHistory.length === 0 && <p>No borrowing history found</p>}
          </div>
        </div>
      </div>
    </div>
